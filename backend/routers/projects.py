...
def create_project(
    ...
    existing_projects = db.query(models.Project).filter(models.Project.owner_id == current_user.id).count()
    plan_limits.enforce_numeric_limit(plan, existing_projects, "max_projects", "projects")
...
def send_email_to_candidate(
    ...
    plan_limits.enforce_feature_access(
        plan_limits.normalize_plan(current_user.plan),
        "can_send_email",
...
def generate_interview_questions(
    ...
    plan_limits.enforce_feature_access(
        plan_limits.normalize_plan(current_user.plan),
        "can_generate_interview_questions",
...
# ── Job Recommendation Generator (with real job links) ────────────────────────

class JobRecommendRequest(BaseModel):
    other_roles: str = ""       # manual fallback roles (newline-separated)
    use_posted_jobs: bool = True # fetch from job_postings table


@router.post("/{project_id}/candidates/{candidate_id}/job-recommendations")
def generate_job_recommendations(
    ...
    """
    Analyses a rejected candidate's skills against open roles and generates
    a personalised recommendation email with real apply links.
    """
    import re, json
    from openai import OpenAI
    ...
    if candidate.status != "rejected":
        raise HTTPException(status_code=400, detail="Job recommendations are only for rejected candidates.")
    ...
    # Fetch active job postings from DB
    posted_jobs = []
    if body.use_posted_jobs:
        posted_jobs = db.query(models.JobPosting).filter(
            models.JobPosting.owner_id == current_user.id,
            models.JobPosting.is_active == True,
        ).all()

    # Manual fallback roles
    manual_roles = [r.strip() for r in (body.other_roles or "").strip().splitlines() if r.strip()]

    if not posted_jobs and not manual_roles:
        raise HTTPException(
            status_code=400,
            detail="No active job postings found. Create job postings first or enter roles manually."
        )

    # Build role list for AI
    role_descriptions = []
    for job in posted_jobs:
        skills_str = ", ".join(job.required_skills or []) or "Not specified"
        role_descriptions.append(
            f"- {job.title} (ID:{job.id}) | Skills: {skills_str} | {job.employment_type} | {job.location or 'Remote/Any'}"
        )
    for r in manual_roles:
        role_descriptions.append(f"- {r}")

    client = OpenAI(api_key=api_key)

    prompt = f"""
You are a senior HR consultant matching a rejected candidate to alternative roles.
...
- Education: {candidate.education_degree or ""} in {candidate.education_field or ""}
- Work History: {candidate.work_history or []}
- Was evaluated for: {project.title}
- Why not selected: {candidate.explanation or "Did not fully meet the primary role requirements"}

AVAILABLE OPEN ROLES:
{chr(10).join(role_descriptions)}

TASK:
1. Select 1-3 roles that genuinely match this candidate (skip poor fits).
2. Write a warm, encouraging email. For each recommended role include the placeholder [APPLY_LINK_<ID>] where <ID> is the job ID from the list above (e.g. [APPLY_LINK_5]). For manual roles without an ID, use [APPLY_LINK_0].
3. The email should feel personal and specific, not generic.

Return ONLY a valid JSON object (no markdown, no backticks):
{{
  "recommended_roles": [
    {{
      "role": "Exact role title from the list",
      "job_id": <integer id or null for manual roles>,
  ...
  "email_body": "Dear [Name],\\n\\n...email body with [APPLY_LINK_<ID>] placeholders...\\n\\nBest regards,\\nTalentAlign Recruitment Team"
}}
"""
    ...
    result["email_body"] = result["email_body"].replace("[Name]", candidate_name)

    # Build ID -> URL map for posted jobs
    job_url_map = {str(job.id): f"{frontend_url}/jobs/{job.id}" for job in posted_jobs}

    # Inject real links into email body
    import re as _re
    def replace_link(m):
        job_id_str = m.group(1)
        url = job_url_map.get(job_id_str, f"{frontend_url}/jobs")
        return f"👉 Apply here: {url}"

    result["email_body"] = _re.sub(r"\[APPLY_LINK_(\w+)\]", replace_link, result["email_body"])

    # Add apply_link to each recommended role
    for rec in result.get("recommended_roles", []):
        jid = rec.get("job_id")
        rec["apply_link"] = job_url_map.get(str(jid), "") if jid else ""

    result["candidate_name"]  = candidate_name
    result["candidate_email"] = candidate.email or ""
    result["has_posted_jobs"] = len(posted_jobs) > 0
    result["total_active_jobs"] = len(posted_jobs)
